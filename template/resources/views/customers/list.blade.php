...
<div class="card shadow mb-4">
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered" id="customers-list-datatable" width="100%" cellspacing="0">
                <thead><tr><th>No</th>
<th>Name</th>
<th>Email</th>
<th>Address</th>
...
document.addEventListener('DOMContentLoaded', function() {
    let table = $('#customers-list-datatable').DataTable({
        processing: true,
        serverSide: true,
...
{ data: 'address', name: 'address', className: 'text-center' },
{
    ...
    render: function(data, type, row) {
        let urlShow = `{{ config('app.url') }}/customers/${row.id}`;
        let urlEdit = `{{ config('app.url') }}/customers/${row.id}/edit`;
        let urlDelete = `{{ config('app.url') }}/customers/${row.id}`;
        return `
