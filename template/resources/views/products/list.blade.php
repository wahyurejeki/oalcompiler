...
<div class="card shadow mb-4">
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered" id="products-list-datatable" width="100%" cellspacing="0">
                <thead><tr><th>No</th>
<th>Sku</th>
<th>Name</th>
<th>Stock</th>
...
document.addEventListener('DOMContentLoaded', function() {
    let table = $('#products-list-datatable').DataTable({
        processing: true,
        serverSide: true,
...
{ data: 'category_name', name: 'category_name', className: 'text-center' },
{
    ...
    render: function(data, type, row) {
        let urlShow = `{{ config('app.url') }}/products/${row.id}`;
        let urlEdit = `{{ config('app.url') }}/products/${row.id}/edit`;
        let urlDelete = `{{ config('app.url') }}/products/${row.id}`;
        return `
