...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Inventorylogs list</h1>
...
<div class="card shadow mb-4">
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered" id="inventorylogs-list-datatable" width="100%" cellspacing="0">
                <thead><tr><th>No</th>
<th>Product</th>
<th>Change_amount</th>
<th>Reason</th>
...
document.addEventListener('DOMContentLoaded', function() {
    let table = $('#inventorylogs-list-datatable').DataTable({
        processing: true,
        serverSide: true,
...
{ data: 'change_amount', name: 'change_amount', className: 'text-center' },
{ data: 'reason', name: 'reason', className: 'text-center' },
{
    ...
    render: function(data, type, row) {
        let urlShow = `{{ config('app.url') }}/inventorylogs/${row.id}`;
        let urlEdit = `{{ config('app.url') }}/inventorylogs/${row.id}/edit`;
        let urlDelete = `{{ config('app.url') }}/inventorylogs/${row.id}`;
        return `
