...
<div class="card shadow mb-4">
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered" id="orders-list-datatable" width="100%" cellspacing="0">
                <thead><tr><th>No</th>
<th>Customer</th>
<th>Order_date</th>
<th>Total_amount</th>
...
document.addEventListener('DOMContentLoaded', function() {
    let table = $('#orders-list-datatable').DataTable({
        processing: true,
        serverSide: true,
...
{ data: 'status', name: 'status', className: 'text-center' },
{
    ...
    render: function(data, type, row) {
        let urlShow = `{{ config('app.url') }}/orders/${row.id}`;
        let urlEdit = `{{ config('app.url') }}/orders/${row.id}/edit`;
        let urlDelete = `{{ config('app.url') }}/orders/${row.id}`;
        return `
