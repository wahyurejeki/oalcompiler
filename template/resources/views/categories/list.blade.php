...
<div class="card shadow mb-4">
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered" id="categories-list-datatable" width="100%" cellspacing="0">
                <thead><tr><th>No</th>
<th>Name</th>
<th>Description</th>
<th>Aksi</th></tr></thead>
...
document.addEventListener('DOMContentLoaded', function() {
    let table = $('#categories-list-datatable').DataTable({
        processing: true,
        serverSide: true,
...
{ data: 'description', name: 'description', className: 'text-center' },
{
    ...
    render: function(data, type, row) {
        let urlShow = `{{ config('app.url') }}/categories/${row.id}`;
        let urlEdit = `{{ config('app.url') }}/categories/${row.id}/edit`;
        let urlDelete = `{{ config('app.url') }}/categories/${row.id}`;
        return `
