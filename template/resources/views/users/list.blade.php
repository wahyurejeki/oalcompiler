...
<div class="card shadow mb-4">
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered" id="users-list-datatable" width="100%" cellspacing="0">
                <thead><tr><th>No</th>
<th>Name</th>
<th>Email</th>
<th>Password</th>
...
{ data: 'password', name: 'password', className: 'text-center' },
{
    ...
    render: function(data, type, row) {
        let urlShow = `{{ config('app.url') }}/users/${row.id}`;
        let urlEdit = `{{ config('app.url') }}/users/${row.id}/edit`;
        let urlDelete = `{{ config('app.url') }}/users/${row.id}`;
        return `
