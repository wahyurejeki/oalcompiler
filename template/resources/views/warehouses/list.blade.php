...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Warehouses list</h1>
...
<div class="card shadow mb-4">
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered" id="warehouses-list-datatable" width="100%" cellspacing="0">
                <thead><tr><th>No</th>
<th>Name</th>
<th>Manager_name</th>
<th>Aksi</th></tr></thead>
...
document.addEventListener('DOMContentLoaded', function() {
    let table = $('#warehouses-list-datatable').DataTable({
        processing: true,
        serverSide: true,
...
{ data: 'manager_name', name: 'manager_name', className: 'text-center' },
{
    data: 'action',
    name: 'action',
    ...
    render: function(data, type, row) {
        let urlShow = `{{ config('app.url') }}/warehouses/${row.id}`;
        let urlEdit = `{{ config('app.url') }}/warehouses/${row.id}/edit`;
        let urlDelete = `{{ config('app.url') }}/warehouses/${row.id}`;
        return `
