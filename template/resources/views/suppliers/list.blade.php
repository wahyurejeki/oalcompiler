...
<div class="card shadow mb-4">
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered" id="suppliers-list-datatable" width="100%" cellspacing="0">
                <thead><tr><th>No</th>
<th>Name</th>
...
document.addEventListener('DOMContentLoaded', function() {
    let table = $('#suppliers-list-datatable').DataTable({
        processing: true,
        serverSide: true,
...
{ data: 'contact_person', name: 'contact_person', className: 'text-center' },
{ data: 'phone', name: 'phone', className: 'text-center' },
{
    ...
    render: function(data, type, row) {
        let urlShow = `{{ config('app.url') }}/suppliers/${row.id}`;
        let urlEdit = `{{ config('app.url') }}/suppliers/${row.id}/edit`;
        let urlDelete = `{{ config('app.url') }}/suppliers/${row.id}`;
        return `
