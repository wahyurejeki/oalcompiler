...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Orderitems list</h1>
...
<div class="card shadow mb-4">
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered" id="orderitems-list-datatable" width="100%" cellspacing="0">
                <thead><tr><th>No</th>
<th>Order</th>
<th>Product</th>
<th>Quantity</th>
...
document.addEventListener('DOMContentLoaded', function() {
    let table = $('#orderitems-list-datatable').DataTable({
        processing: true,
        serverSide: true,
...
{ data: 'price_at_purchase', name: 'price_at_purchase', className: 'text-center' },
{
    data: 'action',
    name: 'action',
    ...
    render: function(data, type, row) {
        let urlShow = `{{ config('app.url') }}/orderitems/${row.id}`;
        let urlEdit = `{{ config('app.url') }}/orderitems/${row.id}/edit`;
        let urlDelete = `{{ config('app.url') }}/orderitems/${row.id}`;
        return `
